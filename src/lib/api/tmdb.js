...
// Tipos de búsqueda de la página de resultados completos (/search) y el
// endpoint de TMDb que le corresponde a cada uno.
...
// TMDb no sirve más allá de la página 500 en sus búsquedas.
export const TMDB_SEARCH_MAX_PAGE = 500;

/**
 * Una página de resultados de búsqueda de TMDb, normalizada para la página de
 * resultados completos: cada elemento lleva `media_type` y el total de páginas
 * se acota al máximo que TMDb acepta.
 *
 * Devuelve `null` si TMDb no está disponible (para poder mostrar "reintentar"
 * en vez de un falso "sin resultados").
 */
export async function searchTmdb(kind, query, { page = 1, language = "es-ES" } = {}) {
  const path = SEARCH_ENDPOINTS[kind] || SEARCH_ENDPOINTS.all;
  const safePage = Math.min(Math.max(1, Math.trunc(Number(page) || 1)), TMDB_SEARCH_MAX_PAGE);
  const json = await tmdb(
    path,
    { query, page: safePage, language, include_adult: "false" },
  ...
  const results = (Array.isArray(json.results) ? json.results : [])
    .map((item) => ({ ...item, media_type: item.media_type || fallbackType }))
    .filter((item) =>
      item?.id != null &&
      ["movie", "tv", "person", "collection"].includes(item.media_type),
    );

  return {
    page: safePage,
    totalPages: Math.min(Number(json.total_pages) || 0, TMDB_SEARCH_MAX_PAGE),
