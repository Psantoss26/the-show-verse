...
export async function fetchRecommendedMovies(sessionId) {
  if (!sessionId) return []
  const data = await tmdb('/account/0/recommendations', { session_id: sessionId })
  return data?.results || []
}
...
export async function getLogos(type, id) {
  ...
  const filtered = logos.filter((l) => ['es', 'en', null].includes(l.iso_639_1))
...
export async function getExternalIds(type, id) {
  const data = await tmdb(`/${type}/${id}/external_ids`, { language: undefined })
...
export async function markAsFavorite({ accountId, sessionId, type, mediaId, favorite }) {
  if (!accountId || !sessionId) throw new Error('Faltan accountId o sessionId para marcar favorito')
...
export async function markInWatchlist({ accountId, sessionId, type, mediaId, watchlist }) {
  if (!accountId || !sessionId) throw new Error('Faltan accountId o sessionId para watchlist')
  ...
  if (!res.ok || data?.success === false) {
    ...
    throw new Error(data?.status_message || 'No se pudo actualizar la lista de pendientes')
  }
  return data
}
...
export async function fetchFavoritesForUser(accountId, sessionId) {
  ...
  const fetchList = async (path, mediaType) => {
    const res = await fetch(
      buildUrl(path, { session_id: sessionId, sort_by: 'created_at.desc' }),
      { cache: 'no-store' }
    )
    const data = await res.json()
...
export async function fetchTopRatedTMDb({ type = 'movie', page = 1, minVotes = 500 } = {}) {
...
export async function fetchTopRatedIMDb({ type = 'movie', pages = 3, limit = 20, minVotes = 10000 } = {}) {
  ...
  const res = await fetch(`/api/imdb/top-rated?${params.toString()}`, { cache: 'no-store' })
  const json = await res.json().catch(() => ({}))
  if (!res.ok) return []
  return json.items || []
}
...
export async function fetchMovieSections({ pageRand = 1, genreIds = [28, 35, 18, 878, 53, 16] } = {}) {
  ...
  const queries = [
    ...
    discoverMovies({ sort_by: 'vote_average.desc', 'vote_count.gte': 1000, page: pageRand }),
    ...
    discoverMovies({ 'vote_average.gte': 7.5, 'vote_count.gte': 2000, sort_by: 'vote_average.desc', page: pageRand }),
    ...
    discoverMovies({ 'vote_count.gte': 5000, sort_by: 'popularity.desc', page: pageRand }),
  ]

  // Por género (varios grupos)
  const genrePromises = genreIds.map((gid) =>
    discoverMovies({ with_genres: gid, sort_by: 'popularity.desc', page: pageRand })
  )

  const [
    mostVoted,
    dec90, dec00, dec10, dec20,
    awardedProxy,
    topES,
    blockbusters,
    ...genresResults
...
export async function fetchTVSections({ pageRand = 1, genreIds = [18, 35, 9648, 10764, 16, 80, 10765] } = {}) {
  const makeDecade = (from, to) =>
    discoverTV({
  ...
  const queries = [
    // Más votadas (tv)
    discoverTV({ sort_by: 'vote_average.desc', 'vote_count.gte': 500, page: pageRand }),
    ...
    // Premiadas (proxy TV)
    discoverTV({ 'vote_average.gte': 7.8, 'vote_count.gte': 1000, sort_by: 'vote_average.desc', page: pageRand }),
    ...
    discoverTV({ 'vote_count.gte': 2000, sort_by: 'popularity.desc', page: pageRand }),
  ]

  // Por género
  const genrePromises = genreIds.map((gid) =>
    discoverTV({ with_genres: gid, sort_by: 'popularity.desc', page: pageRand })
  )
...
export async function fetchMovieBuckets({ pageRand = 1 } = {}) {
  ...
  const queries = [
    ...
    // Idiomas
    discoverMovies({ with_original_language: 'es', sort_by: 'popularity.desc', page: pageRand }),
    discoverMovies({ with_original_language: 'fr', sort_by: 'popularity.desc', page: pageRand }),
    discoverMovies({ with_original_language: 'ja', sort_by: 'popularity.desc', page: pageRand }),

    // Géneros
    discoverMovies({ with_genres: 28, sort_by: 'popularity.desc', page: pageRand }), // Acción
    discoverMovies({ with_genres: 35, sort_by: 'popularity.desc', page: pageRand }), // Comedia
    discoverMovies({ with_genres: 18, sort_by: 'popularity.desc', page: pageRand }), // Drama
    discoverMovies({ with_genres: 878, sort_by: 'popularity.desc', page: pageRand }), // Sci-Fi
  ]

  const [
    classics, y2000s, y2010s, y2020s,
    hiddenGems,
    intlEs, intlFr, intlJa,
...
export async function fetchTVBuckets({ pageRand = 1 } = {}) {
  const queries = [
    ...
    discoverTV({ sort_by: 'vote_average.desc', 'vote_count.gte': 200, page: pageRand }), // Top nota
    discoverTV({ with_genres: 35, sort_by: 'popularity.desc', page: pageRand }),  // Comedia
    discoverTV({ with_genres: 10759, sort_by: 'popularity.desc', page: pageRand }), // Acción/Aventura
    discoverTV({ with_original_language: 'ko', sort_by: 'popularity.desc', page: pageRand }), // K-Drama
    discoverTV({ with_original_language: 'en', sort_by: 'vote_count.desc', page: pageRand }), // Mainstream
    discoverTV({ with_original_language: 'es', sort_by: 'popularity.desc', page: pageRand }), // ES
  ]

  const [
    drama, topRated, comedy, actionAdv, kDrama, mainstream, spanish,
  ] = await Promise.all(queries)
