...
function traktUserAgent() {
  return (
    process.env.TRAKT_USER_AGENT || "TheShowVerse/1.0 (Next.js; Trakt OAuth)"
  );
}
...
export async function exchangeCodeForTokens({ code, redirectUri }) {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        json?.error_description || json?.error || "Trakt token exchange failed",
      );
    }
    ...
    if (err.name === "AbortError") {
      throw new Error("Trakt OAuth timeout");
    }
    throw err;
  }
}
...
export async function traktGetWatchedForShow(token, { traktId }) {
  const r = await traktFetch(
    `/sync/watched/shows/${encodeURIComponent(traktId)}`,
    { token },
  );
...
export function mapWatchedEpisodesBySeason(watchedPayload) {
  const obj = Array.isArray(watchedPayload)
    ? watchedPayload[0]
    : watchedPayload;
...
export async function traktAddEpisodeToHistory(
  token,
  { showTmdbId, season, episode, watchedAtIso },
) {
  const body = {
    shows: [
      {
...
export async function traktRemoveEpisodeFromHistory(
  token,
  ...
  if (!r.ok)
    throw new Error(`Trakt remove episode history failed (${r.status})`);
  return r.json;
}
...
export async function traktGetProgressWatchedForShow(token, { traktId }) {
  ...
  if (!r.ok)
    throw new Error(`Trakt show progress watched failed (${r.status})`);
  return r.json;
}

// ✅ Mapea progress -> { [seasonNumber>=1]: [episodeNumber...] }
export function mapProgressWatchedBySeason(progressPayload) {
  const seasons = Array.isArray(progressPayload?.seasons)
    ? progressPayload.seasons
    : [];
  const out = {};
