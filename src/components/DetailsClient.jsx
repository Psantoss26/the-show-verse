...
export default function DetailsClient({
  ...
  const registerSection = useCallback((sid) => (el) => {
  ...
  const scrollToSection = useCallback(
    (sid) => {
      const el = sectionElsRef.current[sid] || document.getElementById(`section-${sid}`)
      if (!el) return
    ...
    "
      ...
      "
          ...
          >
            <div
              className={[
                'transition-all',
                menuCompact
                  ? 'rounded-2xl border border-white/10 bg-black/65 backdrop-blur-md shadow-lg'
                  : 'rounded-3xl',
              ].join(' ')}
            >
              <DetailsSectionMenu
        ...
        px-4 h-10 md:h-11 text-xs font-bold uppercase tracking-wider transition hover:bg-zinc-200"
            ...
            {collectionId && (
              <section id="section-collection" ref={registerSection('collection')}>
                {/* --- COLECCIÓN --- */}
                <section className="mb-10">
