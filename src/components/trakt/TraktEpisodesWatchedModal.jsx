...
import { Check, Eye, EyeOff, Loader2, X, Search, List, Table2, Filter, ChevronRight } from 'lucide-react'
...
export default function TraktEpisodesWatchedModal({
    open, onClose, mediaType = 'tv', tmdbId, title, connected, seasons = [], watchedBySeason = {}, busyKey = '', onToggleEpisodeWatched, movieWatchedAt = null, onToggleMovieWatched
}) {
    ...
    const totals = useMemo(() => {
        ...
        // Solo contamos vistos de temporadas > 0
    ...
    const loadSeason = async (sn) => {
        ...
        try {
            const res = await fetch(`https://api.themoviedb.org/3/tv/${tmdbId}/season/${sn}?api_key=${TMDB_API_KEY}&language=es-ES`)
    ...
    const selectedSeasonObj = useMemo(() => usableSeasons.find((s) => s?.season_number === activeSeason) || null, [usableSeasons, activeSeason])
    ...
    const filteredEpisodes = useMemo(() => {
        ...
        return base.filter((ep) => {
            return (ep.name || '').toLowerCase().includes(q2) || String(ep.episode_number) === q2
        })
    ...
    // Layout Classes Adaptadas (Full height en móvil, padding safe area)
    const PanelClass = "fixed inset-0 sm:static w-full h-[100dvh] sm:h-[85vh] sm:max-w-5xl bg-[#0b0b0b] sm:rounded-3xl sm:border sm:border-white/10 sm:shadow-2xl overflow-hidden flex flex-col z-[10060] sm:z-auto pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] sm:py-0"
    const ButtonBase = "inline-flex items-center justify-center gap-2 rounded-xl text-sm font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed"

    // --- MOVIE RENDER ---
    if (isMovie) {
        return (
            // ✅ Z-Index 99999 para asegurar que esté encima de todo
            <div className="fixed inset-0 z-[99999] flex items-center justify-center sm:p-4">
                <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={onClose} />
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className={`${PanelClass} sm:max-w-lg sm:h-auto`}>
                    ...
                    <div className="p-5 border-b border-white/10 flex justify-between items-start gap-4 bg-[#0b0b0b]">
                        ...
                        <button onClick={onClose} className="p-2 rounded-full bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white transition"><X className="w-5 h-5" /></button>
                    </div>
                    ...
                    <div className="p-6 space-y-6">
                        {!connected ? (
                            <div className="text-center py-8">
                                ...
                                <Link href="/trakt" className={`${ButtonBase} px-6 py-2.5 bg-emerald-600 text-white hover:bg-emerald-500`}>Ir a Conectar</Link>
                            </div>
                        ) : (
                            <>
                                <div>
                                    <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Fecha y hora (opcional)</label>
                                    <input
                                        type="datetime-local"
                                ...
                                <div className="flex gap-3 pt-2">
                                    <button onClick={onClose} className={`${ButtonBase} flex-1 py-3 bg-zinc-800 text-zinc-300 hover:bg-zinc-700`}>Cancelar</button>
                                    {movieWatched ? (
                                        <button
    ...
    // --- TV RENDER ---
    return (
        // ✅ Z-Index 99999 y fondo oscuro sólido
        <div className="fixed inset-0 z-[99999] flex items-center justify-center sm:p-4">
            <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={onClose} />
            ...
            >
                {/* Header: Fondo Sólido para que no se transparente nada */}
                <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 bg-[#0b0b0b] shrink-0 z-30">
                    ...
                    <button onClick={onClose} className="p-2.5 rounded-full bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white transition">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="px-5 py-3 border-b border-white/5 flex flex-col sm:flex-row gap-3 bg-[#0b0b0b] shrink-0 z-20">
                    ...
                    <div className="flex gap-2 shrink-0 overflow-x-auto no-scrollbar">
                        <div className="flex bg-zinc-900 rounded-xl p-1 border border-white/5">
                            <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition ${viewMode === 'list' ? 'bg-zinc-700 text-white shadow' : 'text-zinc-400 hover:text-white'}`}>
                                <List className="w-3.5 h-3.5" /> Lista
                            </button>
                            <button onClick={() => setViewMode('table')} className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition ${viewMode === 'table' ? 'bg-zinc-700 text-white shadow' : 'text-zinc-400 hover:text-white'}`}>
                                <Table2 className="w-3.5 h-3.5" /> Tabla
                            </button>
                        </div>
                        <button
                            onClick={() => setOnlyUnwatched(!onlyUnwatched)}
                            className={`px-3 py-1.5 rounded-xl border text-xs font-bold flex items-center gap-1.5 transition whitespace-nowrap ${onlyUnwatched ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-zinc-900 border-white/5 text-zinc-400 hover:bg-zinc-800'}`}
                        >
                ...
                <div className="flex-1 min-h-0 flex flex-col md:flex-row overflow-hidden relative">
                    ...
                    {!connected ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                            ...
                            <Link href="/trakt" className={`${ButtonBase} px-6 py-2.5 bg-emerald-600 text-white hover:bg-emerald-500`}>Conectar Trakt</Link>
                        </div>
                    ) : viewMode === 'list' ? (
                        <>
                                ...
                                <div className="p-3 space-y-1">
                                    {usableSeasons.map(s => {
                                        ...
                                        return (
                                            <button
                                                key={sn}
                                                onClick={() => { setActiveSeason(sn); if (Array.isArray(seasonCache?.[sn]?.episodes)) setDisplaySeason(sn) }}
                                            ...
                                            >
                                                <div>
                                                    <div className={`text-sm font-bold ${active ? 'text-emerald-400' : 'text-zinc-300 group-hover:text-white'}`}>{seasonLabelText(sn, s.name)}</div>
                                                    <div className="text-[10px] text-zinc-500 mt-0.5">{watched} / {total} vistos</div>
                                                </div>
                            ...
                            {/* Mobile Season Selector (Horizontal) */}
                                    ...
                                    {usableSeasons.map(s => {
                                        ...
                                        return (
                                            <button
                                                key={sn}
                                                onClick={() => { setActiveSeason(sn); if (Array.isArray(seasonCache?.[sn]?.episodes)) setDisplaySeason(sn) }}
                                                className={`px-4 py-2 rounded-lg text-xs font-bold whitespace-nowrap border transition-all ${active ? 'bg-emerald-500 text-black border-emerald-500 shadow-lg shadow-emerald-500/20' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700 hover:text-white'}`}
                                            >
                                                {sn === 0 ? 'Esp' : `T${sn}`}
                                            </button>
                                        )
                                ...
                                <div className="space-y-3">
                                    {filteredEpisodes.length === 0 && !loadingSeason ? (
                                        ...
                                        filteredEpisodes.map(ep => {
                                            ...
                                            return (
                                                <div key={en} className="group flex gap-3 sm:gap-4 p-2 sm:p-3 rounded-xl bg-zinc-900/30 border border-white/5 hover:bg-zinc-800/50 hover:border-white/10 transition">
                                                    <div className="w-24 sm:w-32 aspect-video bg-zinc-800 rounded-lg overflow-hidden shrink-0 relative">
                                                        {img ? <img src={img} alt="" className="w-full h-full object-cover" loading="lazy" /> : <div className="w-full h-full flex items-center justify-center"><Tv className="w-6 h-6 text-zinc-700" /></div>}
                                                        {watched && <div className="absolute inset-0 bg-emerald-500/20 flex items-center justify-center backdrop-blur-[1px]"><Check className="w-8 h-8 text-emerald-400 drop-shadow-md" /></div>}
                                                    </div>

                                                    <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
                                                        <div className="flex justify-between items-start gap-2">
                                                            <div className="min-w-0">
                                                                <h4 className="text-sm font-bold text-white truncate pr-2">{en}. {ep.name || `Episodio ${en}`}</h4>
                                                            ...
                                                            >
                                                                {busy ? <Loader2 className="w-5 h-5 animate-spin" /> : watched ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                                                            </button>
                                                        </div>
                                                        <p className="text-[11px] text-zinc-400 line-clamp-2 leading-relaxed hidden sm:block">{ep.overview}</p>
                                                    </div>
                                                </div>
                                            )
                                        })
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-4 pb-20 sm:pb-4">
                            {seasonsFilteredForTable.map(s => {
                                ...
                                const { nums, hasMore, remaining } = {
                                    nums: Array.from({ length: Math.min(total, expandedSeason[sn] ? total : MAX_EPISODES_RENDER) }, (_, i) => i + 1),
                                    hasMore: total > MAX_EPISODES_RENDER && !expandedSeason[sn],
                                    remaining: total - MAX_EPISODES_RENDER
                                }
                                ...
                                return (
                                    <div key={sn} className="bg-zinc-900/30 border border-white/5 rounded-2xl p-4">
                                        <div className="flex justify-between items-center mb-3">
                                            ...
                                            <span className="text-xs font-medium text-zinc-500 bg-zinc-900 px-2 py-1 rounded-md border border-white/5">{watchedCount}/{total}</span>
                                        </div>
                                        <div className="flex flex-wrap gap-1.5">
                                            {nums.map(en => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        disabled={busy}
                                                        onClick={() => onToggleEpisodeWatched?.(sn, en)}
                                                        className={`w-9 h-9 rounded-lg text-xs font-bold flex items-center justify-center transition border ${w ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700 hover:text-white'} ${busy ? 'opacity-50' : ''}`}
                                                    >
                                            ...
                                            {hasMore && (
                                                <button onClick={() => setExpandedSeason(p => ({ ...p, [sn]: true }))} className="px-3 h-9 rounded-lg bg-zinc-800 border border-zinc-700 text-xs font-bold text-zinc-400 hover:text-white">
                                                    +{remaining}
                                                </button>
                                            )}
                                        </div>
