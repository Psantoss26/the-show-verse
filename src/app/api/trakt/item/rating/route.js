...
export async function POST(req) {
    try {
        ...
        if (rating == null) {
            ...
            if (!res.ok) return NextResponse.json({ error: json?.error || 'Trakt rating remove error', raw: json }, { status: res.status })
            return NextResponse.json({ ok: true, rating: null })
        }

        // Trakt rating suele ser 1..10 entero
        const r = Number(rating)
        const safe = Number.isFinite(r) ? Math.min(10, Math.max(1, Math.round(r))) : null
        if (!safe) return NextResponse.json({ error: 'Bad rating' }, { status: 400 })

        const res = await fetch(`${TRAKT_BASE}/sync/ratings`, {
            method: 'POST',
            headers: traktHeaders(token),
            body: JSON.stringify({ [payloadKey]: [{ ids: { tmdb: id }, rating: safe }] }),
        })

        const json = await res.json().catch(() => ({}))
        if (!res.ok) return NextResponse.json({ error: json?.error || 'Trakt rating error', raw: json }, { status: res.status })
        return NextResponse.json({ ok: true, rating: safe })
    } catch (e) {
        return NextResponse.json({ error: e?.message || 'Trakt rating error' }, { status: 500 })
    }
}
