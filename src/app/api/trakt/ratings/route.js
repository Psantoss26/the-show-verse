...
// ✅ Si no defines TRAKT_REDIRECT_URI, usamos TRAKT_APP_ORIGIN o VERCEL_URL
function resolveRedirectUri() {
    ...
    return `${cleanOrigin(origin)}/api/trakt/auth/callback`
}
...
async function refreshIfNeeded(auth) {
    ...
    const secure = true // en Vercel siempre HTTPS

    if (newAccess) {
        store.set('trakt_access_token', newAccess, { path: '/', httpOnly: true, sameSite: 'lax', secure })
        store.set('trakt_refresh_token', newRefresh, { path: '/', httpOnly: true, sameSite: 'lax', secure })
        store.set('trakt_expires_at', String(newExpiresAtMs), { path: '/', httpOnly: true, sameSite: 'lax', secure })
    }
...
export async function GET(req) {
    try {
        ...
        if (type === 'movie' || type === 'show') {
            ...
            while (page <= 20) {
                const res = await traktFetch(`/sync/ratings/${pathType}?extended=full&page=${page}&limit=${limit}`, auth.accessToken)
        ...
        if (type === 'season') {
            ...
            if (!Number.isFinite(tmdbId) || !Number.isFinite(seasonNumber)) return json({ error: 'Missing tmdbId/season' }, 400)

            let page = 1
            const limit = 100

            while (page <= 20) {
                const res = await traktFetch(`/sync/ratings/seasons?extended=full&page=${page}&limit=${limit}`, auth.accessToken)
                if (res.status === 401) return json({ error: 'Unauthorized' }, 401)
                if (!res.ok) {
                    const err = await res.json().catch(() => ({}))
                    return json({ error: err?.error || 'Trakt GET ratings failed' }, res.status)
        ...
        if (type === 'episode') {
            ...
            while (page <= 20) {
                const res = await traktFetch(`/sync/ratings/episodes?extended=full&page=${page}&limit=${limit}`, auth.accessToken)
                if (res.status === 401) return json({ error: 'Unauthorized' }, 401)
                if (!res.ok) {
                    const err = await res.json().catch(() => ({}))
                    return json({ error: err?.error || 'Trakt GET ratings failed' }, res.status)
...
export async function POST(req) {
    try {
        ...
        if (type === 'movie' || type === 'show') {
            ...
            const payload = {
                [key]: [
                    isRemove ? { ids: { tmdb: tmdbId } } : { ids: { tmdb: tmdbId }, rating, rated_at: new Date().toISOString() },
                ],
            }

            const res = await traktFetch(endpoint, auth.accessToken, { method: 'POST', body: payload })
            if (res.status === 401) return json({ error: 'Unauthorized' }, 401)
        ...
        if (type === 'season') {
            ...
            const payload = {
                shows: [
                    {
                        ids: { tmdb: tmdbId },
                        seasons: [isRemove ? { number: seasonNumber } : { number: seasonNumber, rating, rated_at: new Date().toISOString() }],
                    },
                ],
            }
        ...
        // EPISODE
        if (type === 'episode') {
            const showTmdbId = Number(body?.tmdbId ?? body?.showId ?? body?.tvId ?? body?.showTmdbId)
            const seasonNumber = Number(body?.season ?? body?.seasonNumber)
                ...
                shows: [
                    {
                        ...
                        seasons: [
                            {
                                number: seasonNumber,
                                episodes: [isRemove ? { number: episodeNumber } : { number: episodeNumber, rating, rated_at: new Date().toISOString() }],
                            },
                        ],
                    },
                ],
