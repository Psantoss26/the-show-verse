...
export async function GET(request) {
    ...
    try {
        ...
        const res = NextResponse.json({
            ...
            stats: {
                ...
                avgProgress: enriched.length > 0
                    ? Math.round(enriched.reduce((s, x) => s + x.pct, 0) / enriched.length)
                    : 0,
                totalEpisodesWatched: enriched.reduce((s, x) => s + x.completed, 0),
                totalEpisodesRemaining: enriched.reduce((s, x) => s + (x.aired - x.completed), 0),
            },
        })
        ...
        const res = NextResponse.json(
            { connected: true, error: e?.message || 'Error cargando series en progreso', items: [] },
            { status: 500 }
        )
        if (refreshedTokens) setTraktCookies(res, refreshedTokens)
        return res
