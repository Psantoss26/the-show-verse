...
async function resolveShowTraktId(token, tmdbId) {
  const r = await traktFetch(
    `/search/tmdb/${encodeURIComponent(tmdbId)}?type=show`,
    { token },
  );
  if (!r.ok)
    throw new Error(r?.json?.error || `Trakt search failed (${r.status})`);
...
async function fetchShowSeasonsWithEpisodes(token, traktId) {
  const r = await traktFetch(
    `/shows/${encodeURIComponent(traktId)}/seasons?extended=episodes`,
    { token },
  );
  if (!r.ok)
    throw new Error(r?.json?.error || `Trakt seasons failed (${r.status})`);
...
async function fetchAllShowHistory(token, traktId, { startAtIso } = {}) {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    const r = await traktFetch(
      `/sync/history/shows/${encodeURIComponent(traktId)}?${qs.toString()}`,
      { token },
    );
    if (!r.ok)
      throw new Error(
        r?.json?.error || `Trakt show history failed (${r.status})`,
      );
 ...
 */
function computeWatchedBySeasonSince(
  historyItems,
  startAtIso,
  episodeKeySet,
  endBeforeIso = null,
  ...
  const endMs = endBeforeIso ? new Date(endBeforeIso).getTime() : NaN;
  const hasEnd = Number.isFinite(endMs);

  const map = {};
  const historyIdsByEpisode = {};
  const historyIds = [];
...
export async function GET(request) {
  ...
  try {
    ...
    const { showPlays, minPlays } = computeShowCompletions(
      historyAll,
      episodeKeySet,
    );
...
export async function POST(request) {
  ...
  try {
    ...
    for (const seasonsChunk of batches) {
      ...
      if (!r.ok)
        throw new Error(
          r?.json?.error || `Trakt sync/history failed (${r.status})`,
        );
    }
