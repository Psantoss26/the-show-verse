...
export function assertTraktEnv() {
  ...
  if (!process.env.TRAKT_CLIENT_SECRET)
    throw new Error("Missing TRAKT_CLIENT_SECRET");
  if (!process.env.TRAKT_REDIRECT_URI)
    throw new Error("Missing TRAKT_REDIRECT_URI");
}
...
export function setTraktCookies(res, tokenPayload) {
  ...
  // ✅ IMPORTANTE: Convertir a MILISEGUNDOS para consistencia con server.js
  const expiresAtMs =
    createdAt && expiresIn ? (createdAt + expiresIn) * 1000 : 0;
  ...
  if (access) {
    res.cookies.set("trakt_access_token", access, {
      ...common,
      maxAge: Math.max(60, expiresIn || 7776000), // Default 90 días si no hay expiresIn
    });
  }
  if (refresh) {
  ...
  if (expiresAtMs) {
    // ✅ Guardar en MILISEGUNDOS (compatible con readTraktCookies de server.js)
