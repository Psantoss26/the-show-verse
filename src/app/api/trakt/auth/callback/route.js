...
function traktUserAgent() {
  return (
    ...
    "TheShowVerse/1.0 (Next.js; Trakt OAuth Callback)"
  );
}
...
function resolveWebRedirectUri(origin, req) {
  const fromCookie = cleanUrl(
    req.cookies.get("trakt_oauth_redirect_uri")?.value || "",
  );
...
export async function GET(req) {
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.json(
      { error: "Missing TRAKT_CLIENT_ID/SECRET" },
      { status: 500 },
    );
  }
      ...
      return NextResponse.json(
        {
          ...
          message:
            "Has realizado demasiadas peticiones a Trakt. Por favor espera 1 hora antes de intentar de nuevo.",
  ...
  res.cookies.set("trakt_oauth_next", "", {
    httpOnly: true,
    sameSite: "lax",
    secure,
    path: "/",
  ...
  res.cookies.set("trakt_oauth_redirect_uri", "", {
    httpOnly: true,
    sameSite: "lax",
    secure,
    path: "/",
