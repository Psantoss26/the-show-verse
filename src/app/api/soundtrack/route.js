...
function isExactTitleAlbum(album, ctx) {
  ...
  if (!name) return false;
  return ctx.titles.some((title) => norm(title) === name);
}
...
function isCredibleFirstExactTitleAlbum(album, ctx) {
  if (!isAcceptableFirstAlbum(album, ctx)) return false;
  if (!isExactTitleAlbum(album, ctx)) return false;

  const totalTracks = Number(album?.total_tracks ?? 0);
  const artistText = norm(names(album?.artists).join(", "));
  return (
    totalTracks >= 8 &&
    (
      album?.album_type === "compilation" ||
      /various artists|motion picture|soundtrack|score|composer/.test(artistText)
    )
  );
}
