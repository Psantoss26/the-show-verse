  ...
  try {
    ...
    if (!tmdbId || typeof ratingRaw !== "number") {
      return NextResponse.json(
        { error: "Missing mediaType/tmdbId/rating" },
        { status: 400 },
      );
    }

    // TMDb acepta 0.5..10 en pasos 0.5
    const rating = Math.max(0.5, Math.min(10, ratingRaw));
    ...
    if (!sessionId) {
      return NextResponse.json(
        { error: "Not authenticated with TMDb" },
        { status: 401 },
      );
    }
...
export async function DELETE(req) {
  try {
    ...
    if (!tmdbId) {
      return NextResponse.json(
        { error: "Missing mediaType/tmdbId" },
        { status: 400 },
      );
    }
    ...
    if (!r.ok) {
      return NextResponse.json(
        {
          error: data?.status_message || "TMDb remove rating failed",
          details: data,
        },
        { status: r.status },
      );
