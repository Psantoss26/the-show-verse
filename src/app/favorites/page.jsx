...
async function fetchAccountListPageBilingual({
  accountId,
  sessionId,
  listKind,
  mediaType,
...
async function getUnifiedUserRatingCached({
  mediaType,
  id,
  sessionId,
  getImdbId,
  ...
  const p = (async () => {
    ...
    try {
      ...
      if (typeof res?.connected === "boolean")
        traktConnectedKnown = res.connected;
...
async function getTraktCommunityScoreCached({ mediaType, id }) {
  ...
  const p = (async () => {
    try {
      const r = await traktGetScoreboard({
        type: toTraktType(mediaType),
        tmdbId: id,
      });
      const rating =
...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    nextMoviePageRef.current = nextMoviePage;
  }, [nextMoviePage]);
  useEffect(() => {
    nextTvPageRef.current = nextTvPage;
  }, [nextTvPage]);
  useEffect(() => {
    hasMoreMovieRef.current = hasMoreMovie;
  }, [hasMoreMovie]);
  useEffect(() => {
    hasMoreTvRef.current = hasMoreTv;
  }, [hasMoreTv]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      Object.values(imdbTimersRef.current).forEach((t) =>
  ...
  // External IDs helper (shared for IMDb + Trakt fallback)
  ...
  const prefetchImdb = useCallback(
    async (item, opts = {}) => {
      ...
      if (
        hasOwn(imdbRatingsRef.current, key) ||
        imdbInFlightRef.current.has(key)
      )
        return;
      ...
      const run = async () => {
        try {
          ...
          if (cached?.imdbRating != null) {
            if (!unmountedRef.current)
              setImdbRatings((p) => ({ ...p, [key]: cached.imdbRating }));
      ...
      if (delayMs > 0) {
        imdbTimersRef.current[key] = window.setTimeout(run, delayMs);
      } else {
        await run();
      }
    },
  ...
  const prefetchMyRating = useCallback(
    async (item) => {
      ...
      if (hasOwn(myRatingsRef.current, key) || myInFlightRef.current.has(key))
        return;
  ...
  const prefetchTraktScore = useCallback(async (item) => {
    ...
    const key = makeKey(mediaType, item.id);
    if (
      hasOwn(traktScoresRef.current, key) ||
      traktInFlightRef.current.has(key)
    )
      return;

    traktInFlightRef.current.add(key);
    try {
      const payload = await getTraktCommunityScoreCached({
        mediaType,
        id: item.id,
      });
      if (!unmountedRef.current)
        setTraktScores((p) => ({ ...p, [key]: payload?.v ?? null }));
    } finally {
  ...
  const processedItems = useMemo(() => {
    ...
    if (typeFilter !== "all")
      list = list.filter((i) => i.media_type === typeFilter);
    ...
    list.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "imdb_desc": {
          ...
          const ia = getImdb(a);
          return (
            (typeof ib === "number" ? ib : -1) -
            (typeof ia === "number" ? ia : -1)
          );
        }
        case "imdb_asc": {
          const ib = getImdb(b);
          const ia = getImdb(a);
          return (
            (typeof ia === "number" ? ia : 999) -
            (typeof ib === "number" ? ib : 999)
          );
        }
        ...
        case "trakt_desc": {
          ...
          const ra = getTrakt(a);
          return (
            (typeof rb === "number" ? rb : -1) -
            (typeof ra === "number" ? ra : -1)
          );
        }
        case "trakt_asc": {
          const rb = getTrakt(b);
          const ra = getTrakt(a);
          return (
            (typeof ra === "number" ? ra : 999) -
            (typeof rb === "number" ? rb : 999)
          );
        }
        ...
        case "my_desc": {
          ...
          const ma = getMy(a);
          return (
            (typeof mb === "number" ? mb : -1) -
            (typeof ma === "number" ? ma : -1)
          );
        }
        case "my_asc": {
          const mb = getMy(b);
          const ma = getMy(a);
          return (
            (typeof ma === "number" ? ma : 999) -
            (typeof mb === "number" ? mb : 999)
          );
        }
  ...
  const canFetchRemote = useMemo(() => {
    const wantMovie =
      (typeFilter === "all" || typeFilter === "movie") && hasMoreMovie;
  ...
  const ensureAllFavoritesLoaded = useCallback(async () => {
    const sessionId = getSessionId(session);
    if (!sessionId || !account?.id) return;
    ...
    try {
      ...
      while (mHas || tHas) {
        ...
        for (const r of results) {
          if (r.kind === "movie") {
            mHas = r.res.page < r.res.total_pages;
            mPage = r.res.page + 1;
          } else {
            tHas = r.res.page < r.res.total_pages;
            tPage = r.res.page + 1;
          }
        }
  ...
  useEffect(() => {
    if (!wantsFullCollection) return;
    if (loading) return;
    ensureAllFavoritesLoaded();
  }, [wantsFullCollection, ensureAllFavoritesLoaded, loading]);
  ...
  useEffect(() => {
    ...
    runPool(
      processedItems.filter(
        (it) => !hasOwn(imdbRatingsRef.current, makeKey(it.media_type, it.id)),
      ),
      2,
  ...
  useEffect(() => {
    ...
    runPool(
      processedItems.filter(
        (it) => !hasOwn(traktScoresRef.current, makeKey(it.media_type, it.id)),
      ),
      4,
  ...
  const groupedSections = useMemo(() => {
    ...
    const keyToScoreBucket = (prefix, score, mode = "int") => {
      if (typeof score !== "number")
        return { key: "no", label: `${prefix} · Sin nota` };
      if (mode === "half") {
    ...
    for (const it of visibleItems) {
      ...
      if (groupBy === "year") {
        ...
        if (st.status === "pending") {
          gKey = "pending";
          gLabel = "IMDb · Cargando...";
        } else if (st.status === "none") {
          gKey = "no";
          gLabel = "IMDb · Sin nota";
        } else {
        ...
        if (st.status === "pending") {
          gKey = "pending";
          gLabel = "Trakt · Cargando...";
        } else if (st.status === "none") {
          gKey = "no";
          gLabel = "Trakt · Sin nota";
        } else {
        ...
        if (st.status === "pending") {
          gKey = "pending";
          gLabel = "Mi nota · Cargando...";
        } else if (st.status === "none") {
          gKey = "no";
          gLabel = "Mi nota · Sin nota";
        } else {
  ...
  const renderCard = useCallback(
    (item, index) => {
      ...
      const wrapAspect =
        coverMode === "backdrop" ? "aspect-[16/9]" : "aspect-[2/3]";
      const hoverShadow =
        "transition-all duration-500 [@media(hover:hover)]:group-hover:shadow-[0_0_25px_rgba(255,255,255,0.08)]";
      ...
      return (
        ...
        >
          ...
          >
            ...
            >
              <SmartPoster
                key={`${mediaType}:${item.id}:${coverMode}`}
                item={item}
                title={title}
                mode={coverMode}
              />
              ...
              <div className={`${overlayBase} ${overlayOpacity}`}>
                ...
                >
                  <div className="flex items-end justify-between gap-3">
                    ...
                    {myLabel ? (
                      <div
                        className={`shrink-0 self-end transition-all duration-300 ${myTransform}`}
                      >
  ...
  if (authStatus === "anonymous") {
    return (
      <div className="min-h-screen bg-[#101010] max-w-7xl mx-auto px-4 py-20 flex flex-col items-center justify-center text-center">
        ...
        <Link
          href="/login"
          className="px-6 py-2 bg-white text-black rounded-full font-bold hover:bg-neutral-200"
        >
          Iniciar Sesión
        </Link>
      </div>
        ...
        >
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-red-400 font-bold uppercase tracking-widest text-xs">BIBLIOTECA</span>
            </div>
            ...
            <p className="mt-2 text-zinc-400 max-w-lg text-lg">
              Tu selección de películas y series preferidas.
            </p>
          </div>
