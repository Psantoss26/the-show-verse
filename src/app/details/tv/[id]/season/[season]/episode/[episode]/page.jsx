...
export default async function EpisodePage({ params }) {
    ...
    if (!Number.isFinite(showId) || !Number.isFinite(seasonNumber) || !Number.isFinite(episodeNumber)) {
        return (
    ...
    const [show, episode] = await Promise.all([
        ...
        tmdbFetch(`/tv/${showId}/season/${seasonNumber}/episode/${episodeNumber}?append_to_response=credits,external_ids`)
    ])

    const showImdbId = show?.external_ids?.imdb_id || null
    const episodeImdbId = episode?.external_ids?.imdb_id || showImdbId || null

    const omdb = await omdbFetch(episodeImdbId)

    const imdb = episodeImdbId
        ? {
            id: episodeImdbId,
