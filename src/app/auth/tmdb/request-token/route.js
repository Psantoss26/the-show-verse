...
export async function GET(req) {
  console.log('== Iniciando login TMDB ==')
  const TMDB = 'https://api.themoviedb.org/3'
  const API_KEY = process.env.NEXT_PUBLIC_TMDB_API_KEY
  ...
  const tokenUrl = `${TMDB}/authentication/token/new?api_key=${API_KEY}`
  console.log('Solicitando request_token a', tokenUrl)

  const tokenRes = await fetch(tokenUrl, { cache: 'no-store' })
  const tokenData = await tokenRes.json()
  console.log('Respuesta TMDB:', tokenRes.status, tokenData)

  if (!tokenRes.ok || !tokenData.success) {
    return new Response(JSON.stringify({ error: tokenData.status_message || 'Fallo en TMDB' }), {
      status: tokenRes.status || 500,
