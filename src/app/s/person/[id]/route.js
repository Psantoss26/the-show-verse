...
function pickProfile(person) {
    ...
    return person?.profile_path ? `https://image.tmdb.org/t/p/w780${person.profile_path}` : null
}
...
export async function GET(_req, ctx) {
    ...
    // ✅ Todo apunta a /details/person/:id
    const detailsUrl = `${baseUrl}/details/person/${encodeURIComponent(id)}`
    ...
    const description =
        shortDesc(person?.biography) ||
        (person?.known_for_department ? `${name} · ${person.known_for_department}` : `Ver detalles de ${name}.`)

    const profile = pickProfile(person)

    // Para persona no hay backdrop: solo profile (y WhatsApp puede recortar, pero es lo máximo sin generar OG)
    const ogImages = profile ? [{ url: profile, w: 780, h: 1170, type: 'image/jpeg' }] : []
    const twitterImage = profile || ''
