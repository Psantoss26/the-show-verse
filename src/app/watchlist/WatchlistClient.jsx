...
function buildWatchlistGroupMetas(
  item,
  groupBy,
  { imdbScores, traktScores, providersByItem, ratingStep = 0.5, ratingOffset = 0 },
) {
...
function SubGroupDivider({ title, count }) {
  return (
    <div className="flex items-center gap-3 py-1.5 sm:py-2">
      ...
      <div className="inline-flex max-w-[70%] items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/10 px-3 py-1 text-xs sm:text-sm">
        <span className="truncate font-black uppercase tracking-wide text-blue-100">
          {title}
        </span>
        <span className="shrink-0 text-[10px] font-bold text-blue-300/80">
          {count}
        </span>
      </div>
...
export default function WatchlistClient() {
  ...
  const [subGroupBy, setSubGroupBy] = useState(() => {
    ...
    const saved = window.localStorage.getItem(
      "showverse:watchlist:subGroupBy",
    );
    return saved || "none";
  });
  ...
  if (!session || !account) {
    return (
      <div className="min-h-screen bg-[#050505] text-zinc-100 font-sans selection:bg-blue-500/30 pb-20">
          ...
          <div className="flex items-center justify-center py-12 lg:py-24">
            ...
            >
              ...
              <p className="text-zinc-400 max-w-sm mb-8 text-sm">
                Conecta tu cuenta de TMDb para ver y gestionar tu lista de títulos pendientes sincronizada.
              </p>
              <Link
