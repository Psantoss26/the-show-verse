  ...
  const loadWatchlist = useCallback(async () => {
    ...
    try {
      const responses = await Promise.all([
        fetch(
          `/api/tmdb/account/watchlist?type=movies`,
        ).then((r) => (r.ok ? r.json() : { results: [] })),
        fetch(
          `/api/tmdb/account/watchlist?type=tv`,
        ).then((r) => (r.ok ? r.json() : { results: [] })),
      ]);
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 font-sans selection:bg-emerald-500/30">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-[-10%] right-1/4 w-[600px] h-[600px] bg-amber-500/5 rounded-full blur-[150px]" />
      </div>
          ...
          <div className="flex items-center justify-between gap-4 mb-8">
            <div>
              ...
              <h1 className="text-4xl md:text-6xl font-black tracking-tighter text-white">
                Pendientes<span className="text-amber-500">.</span>
              </h1>
              <p className="mt-2 text-zinc-400 max-w-lg text-lg">
                Películas y series que quieres ver próximamente.
              </p>
            </div>
          </div>
        ...
        >
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-zinc-500">No hay pendientes</p>
            </div>
          ) : (
