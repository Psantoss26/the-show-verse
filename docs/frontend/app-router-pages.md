...
| `api/netflix/**` | `connect`, `disconnect`, `pair-mobile`, `poll`, `import`, `simulate`, `extension-sync`, `extension-import`, `extension-progress` | Sincronización de visionado de Netflix vía extensión de navegador y notificaciones móviles (`NetflixSyncListener`); `extension-*` quedan públicas en el middleware por autenticarse con Bearer token propio. |
